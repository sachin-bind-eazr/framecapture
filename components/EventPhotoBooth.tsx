  ...
  return <main className="shell">
    <div className={`booth booth--${stage}`}>
      ...
      {stage === "intro" && <section className="intro screen-enter" aria-labelledby="intro-title">
        <div className="eyebrow"><span className="eyebrow-dot" /> TOGETHER FOR GOOD <span className="eyebrow-dot" /></div>
        ...
        <div className="intro-copy"><p className="kicker">THE EVENT PHOTO BOOTH</p><h1 id="intro-title">Capture<br/><em>the Moment.</em></h1><p className="subtitle">{EVENT_CONFIG.subtitle}</p></div>
      ...
      {(stage === "requesting" || stage === "camera" || stage === "processing") && <section className="camera-screen screen-enter" aria-label="Camera">
        <header className="camera-header"><span className="brand-mark">t<span>g</span></span><div><span className="camera-overline">TOGETHER FOR GOOD</span><strong>Capture the Moment</strong></div><span className="header-spark" aria-hidden="true">✳</span></header>
      ...
      {stage === "preview" && photo && <section className="preview-screen screen-enter" aria-label="Photo preview"><header className="preview-header"><span className="brand-mark">t<span>g</span></span><div><span className="camera-overline">YOUR MOMENT IS READY</span><strong>Looking good!</strong></div><span className="header-spark" aria-hidden="true">✳</span></header><div className="preview-main"><img className="result-photo" src={photo.url} alt="Your photo with the Together for Good event frame" style={{ aspectRatio: `${EVENT_CONFIG.outputWidth} / ${EVENT_CONFIG.outputHeight}` }} /></div><div className="preview-actions"><p className="preview-caption">Share your moment <span>💚</span></p><button className="button button--primary" aria-label="Share photo" onClick={() => void share()}>Share Photo <Icon name="arrow" /></button><button className="button button--secondary" aria-label="Download photo" onClick={save}><Icon name="download" /> Save Photo</button>{shareFallback && <p className="fallback-note" role="status">Save the photo, then share it on Instagram, WhatsApp or Facebook.</p>}<button className="text-action retake" aria-label="Retake photo" onClick={retake}>Retake</button></div></section>}

      {stage === "error" && <section className="error-screen screen-enter" aria-labelledby="error-title"><span className="brand-mark brand-mark--large">t<span>g</span></span><div className="error-icon" aria-hidden="true">✳</div><p className="kicker">LET’S TRY ANOTHER WAY</p><h1 id="error-title">Your moment<br/><em>is still waiting.</em></h1><p className="error-message" role="alert">{error}</p><div className="error-actions"><button className="button button--primary" onClick={() => void openCamera()}>Try Again <Icon name="arrow" /></button><button className="button button--secondary" onClick={() => inputRef.current?.click()} disabled={!frameReady}><Icon name="upload" /> Upload Photo Instead</button></div><p className="privacy">Photos are processed on your device.</p></section>}
